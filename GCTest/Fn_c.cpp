...
int Test0 (short a, short b, short c, short d, short e)
	{
	a = a + 1;
	b = b + 1;
	c = c + 1;
...
// WinPrintf is a replacement for printf that will send output to the output window (edit ctl in this case)
// WinPrintfSetup needs to be called first to setup the output window handle
//
static CEdit* pcOUT_WND = NULL;

void WinPrintfSetup (CEdit* pcOutWnd)
   {
   pcOUT_WND = pcOutWnd;
   }

int WinPrintf (PSZ psz, ...)
   {
	va_list vlst;
   char szBuff[512];
   ...
   // todo:  replace \n with \r\n in szBuff
	...
	pcOUT_WND->SetSel (-1, -1);
	pcOUT_WND->ReplaceSel (szBuff);
...
void InitCFunctions (void)
   ...
   AddFn (WinPrintf      , DATATYPE_INT, 110);
...
// AddFn (KeyEditCellMode, DATATYPE_VOID, 407);
   ...
   AddFn (Test0          , DATATYPE_INT, 800);
   AddFn (Test999        , DATATYPE_PTR, 999);
   }
